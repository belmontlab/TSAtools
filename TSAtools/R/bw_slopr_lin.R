#'TSA slopr with linear fit
#'
#'This Function approximates TSA signal slope (or any other continous genomic signal) by a linear (degree 1) polynomial.
...
#' @export
#' @example
#' x <- BW_slopr(bigwig, bin=NA,slop_wind)
#'
#'
BW_slopr_lin<-function(BW,bin,slop_wind){
  ...
  slop<-function(x){
    ...
    if (any(is.na(x))|any(x==0)){
      ...
      c<-lm(x~poly(1:(wind+1),1,raw = TRUE))$coefficients
      return(abs(c[2]*((wind/2)+1)))
    }
